...
class Metric():
    ...
    def dice_for_case(self):
        ...
        for organ_index in range(self.num_class):
            try:
                pd = self.y_pred_case == organ_index
                gt = self.y_true_case == organ_index
                dice_coefficient = 2 * np.logical_and(pd, gt).sum() / (
                        pd.sum() + gt.sum() + 1e-5
                )
            except ZeroDivisionError:
                print("You can't divide by zero!")
            out[organ_index] = dice_coefficient
